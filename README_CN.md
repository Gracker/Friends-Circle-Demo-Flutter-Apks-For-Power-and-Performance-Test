...
> **设计参考 [HighPerformanceFriendsCircle](https://github.com/Gracker/Friends-Circle-Demo-Apks-For-Power-and-Performance-Test) 原生项目。**
> 使用渐进负载、概率触发、帧间隔控制和滚动阶段感知。

### Build负载迭代次数（帧内，比例 1:4:10）
| 级别 | 迭代次数 |
|-----|---------|
| 轻 | 2,000 |
| 中 | 8,000 |
...
### Mixed负载组合（Build + PostFrame = 帧内总量）
| 级别 | Build部分 | PostFrame部分 | 总量 |
|-----|----------|---------------|------|
| 轻 | 1,000 | 1,000 | 2,000 |
| 中 | 4,000 | 4,000 | 8,000 |
| 重 | 10,000 | 10,000 | 20,000 |
...
- 按压拖动/手指滑动区间：Build、Paint、PostFrame、Mixed 都不执行负载。
- 松手后的惯性滑动区间：才按上面的概率和帧间隔执行负载。
- 静止区间：立即停止负载。
